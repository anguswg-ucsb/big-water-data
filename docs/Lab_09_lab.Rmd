...
ggplotly(gg), tooltip = c('text2')) %>%
  style(hoverlabel = label) %>%
  layout(font = font) %>% 
  config(displaymode = FALSE)

highcharter::highchart() %>% 
  highcharter::hc_add_series(wu_all_sectors, highcharter::hcaes(x = Year, y = Withdrawals, group = "Sectors"), type = "column")
...
leaflet() %>%
  ...
  addPolygons(data = source_GW_2015,
              color = 'black', group = 'GW')
  addPolygons(data = aquifer2, fillColor = ~pals4(AQ_NAME), fillOpacity = 0.3,
              color = 'black',
              label = ~AQ_NAME, group = 'SW')
 addLayersControl(overlayGroups = c('AMA', 'Aquifer'),
